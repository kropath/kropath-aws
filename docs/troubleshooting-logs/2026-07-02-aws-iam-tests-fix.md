...
Root causes fixed in `rgds/awsiamrole.yaml`:
1. Naming template — implemented via `.replace()` which works in kro's CEL. Template `{namespace}-{name}` in `awsiamrole` namespace produces `awsiamrole-{rolename}`.
...
`crds/awsiamconfig.yaml`: Added `addSsmPolicy`, `addEcsExecutionPolicy`, `addLambdaBasicPolicy` to `status.effectiveConfig.mandatory` and `status.effectiveConfig.defaults`.

Test files updated:
- `chainsaw-test.yaml` — all `default-*` name assertions → `awsiamrole-*`; boundary-optout fix; seed patch with full policy flags
- `05-policy-document.yaml`, `06-policy-document-trust.yaml`, `08-my-policy.yaml` — added `kropath.run/resource-name` labels
...
`rgds/awsiamrole.yaml`: In the `tags` block of the `role` template, changed `syncedLabels.transformList(k, v, {"key": "kropath.run/" + k, "value": v})` back to `syncedLabels.transformList(k, v, {"key": k, "value": v})` (same for `syncedAnnotations`). K8s `metadata.labels`/`metadata.annotations` still use the prefixed form.

`tests/iam/awsiamrole/chainsaw-test.yaml`: In the `lambda-role-standard-metadata` step, removed the `kropath.run/` prefix from expected cloud tag keys — assertions now expect plain `environment: production` and `team: platform`.

Result: `kubectl get roles.iam.services.k8s.aws lambda-role -n awsiamrole -o jsonpath='{.spec.tags}'` returns `[{"key":"environment","value":"production"},{"key":"team","value":"platform"}]` — matches the fixed assertion exactly.
...
`rgds/awsiamrole.yaml`: Rewrote the `tags` block to merge maps first (mandatory wins via last-writer-wins in `.merge()`), then convert to the list format at the very end:
...
Result on a fresh awsiamconfig (mandatory.tags = `{cost-centre:platform, environment:mandatory}`, spec.tags = `{app:payments, environment:instance}`, defaults.tags = `{environment:defaults, team:ops}`):
...
Each of the following steps was reproduced manually against a fresh `awsiamconfig` seed and matches its chainsaw assertion. The tag-merge rewrite (Step 3) is compatible with every step, and no other RGD change was needed.

| Step | Verified output |
|---|---|
| `ec2-role-without-ssm-policy` | `name=awsiamrole-ec2-no-ssm-role policies=[]` |
| `ec2-role-without-instance-profile` | `name=awsiamrole-ec2-no-instance-profile-role`; no InstanceProfile created |
...
| `eks-role-variants` (pod-identity) | `PodIdentityAssociation` with `roleARN: arn:aws:iam::123456789012:role/awsiamrole-podid-role` |
...
| `default-naming-template-sets-resource-name` | `resourceName=awsiamrole-lambda-exec` |
| `mandatory-naming-template-wins` | `resourceName=awsiamrole-mandatory-name-template-role-123456789012` |
...
All steps of the `iam/awsiamrole` chainsaw suite are now expected to pass end-to-end when `make test-iam` is run. If a fresh run reveals any new failure, capture the error and treat it as the next step to fix.

---

## awsiamidentityprovider suite — test fixes (2026-07-02)
...
**Fix:** Added `kubectl delete awsiamconfig general-policy -n default --ignore-not-found=true` before re-applying `00-awsiamconfig.yaml` in `seed-effective-config`, `synced-labels`, and `standard-metadata` steps (delete-first pattern from `docs/frequent-rgd-errors.md`).
...
**Fix:** Removed the `AWSIAMIdentityProvider` status assertion from the `saml-provider-arn` step entirely. The step now only asserts the advisory ConfigMap (`02-assert-saml-provider.yaml`), which is the correct observable artifact for SAML type.

---

## awsiamidentityprovider — alignment with awsiamrole RGD (2026-07-02)
...
`awsiamrole` computes `effectiveName` via mandatory template → defaults template → `nameOverride` → `metadata.name`. `awsiamidentityprovider` hardcoded `schema.metadata.name` for the child resource name.
...
`status.namingStatus` returned `"active"` unconditionally. Added unresolved-token check: `oidcProvider.metadata.name.contains("{") ? "invalid-unresolved-tokens" : "active"`, matching awsiamrole.

---

### Missing `additionalPrinterColumns`

Added Type, ResourceName, NamingStatus, ValidationError columns to match awsiamrole's kubectl column output pattern.

---

### New tests: `naming-template`, `tag-merge-prefers-mandatory-values`, `mandatory-naming-template-wins`

Three new chainsaw steps added to cover the same scenarios as awsiamrole:
...
## Full field alignment audit: awsiamidentityprovider vs awsiamrole (2026-07-02)

Checked every field category for cross-RGD consistency.

| Field | awsiamrole | awsiamidentityprovider | Result |
|---|---|---|---|
...
## awsiamconfig-schema-validation suite — `reject-max-session-duration-below-floor` (2026-07-03)
...
Used the in-graph ConfigMap advisory pattern (same as `urlValidationError` in awsiamidentityprovider).

**`rgds/awsiamrole.yaml`:** Added `maxSessionDurationError` ConfigMap resource included only when `schema.spec.maxSessionDuration > 0 && schema.spec.maxSessionDuration < 900`. Added `status.validationError` field and `ValidationError` printer column.
...
## awsiampolicy suite — full suite fix (2026-07-03)

All 10 steps now pass (`chainsaw/awsiampolicy PASS`). Changes fell into four areas:
...
`status.namingStatus` extended to check for unresolved tokens: `contains("{") ? "invalid-unresolved-tokens" : ...` (matching `awsiamrole` pattern).
...
Fixed `policy.spec.name` to follow the same mandatory → defaults → `metadata.name` cascade as `awsiamrole` (using `.replace("{namespace}", ...)` and `.replace("{name}", ...)`).
...
`02-awspolicydocument.yaml`: added `labels: {kropath.run/resource-name: my-policy-doc}` so the sentinel selector can find it.
...
Replaced with the standard `.merge()` map chain from `awsiamrole` (defaults → spec → mandatory, last-writer-wins), then `.transformList(k, v, {"key": k, "value": v})` at the end. All six tier sub-keys included: `syncedAnnotations`, `syncedLabels`, `tags` per tier.
...
### awsiamidentityprovider — `synced-labels` cloud tag prefix bug
...
### awsiamrole — `tag-merge-prefers-mandatory-values` wrong assert order
...
- **Fix**: Reordered `tests/iam/awsiamrole/chainsaw-test.yaml` assert inline tags to match CEL output order.

### awsiamrole — `policy-arn-reference` spec.policies empty
...
### awsiamrole-standard-metadata — namespace delete hang
...
### awsiamrole-standard-metadata — Role never materializes
...
### awsiamidentityprovider — `tag-merge-prefers-mandatory-values` wrong assert order
- **Root cause**: Same CEL merge ordering as awsiamrole above. Assert file `11-assert-oidc-tag-merge.yaml` expected `[app, cost-centre, environment, team]` but actual is `[cost-centre, environment, app, team]`.
- **Fix**: Reordered assert tags.

### awsiamidentityprovider — `retain-delete-policy` / `delete-delete-policy` resource not found
- **Root cause**: Both steps ran after `mandatory-naming-template-wins` which set `mandatory.namingTemplate: "{namespace}-{name}-{account_id}"`. Without a config reset, `retain-oidc` gets named `awsiamidentityprovider-retain-oidc-123456789012` — assert expects `retain-oidc`.
...
### What failed
`awsiamrole-permissionsboundary-mandatory-wins` — `04-expected-role.yaml` expected tag order `[cost-center, team, environment, managed-by]` but actual CEL output was `[cost-center, managed-by, team, environment]`.
