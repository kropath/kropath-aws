...
- **Never assert a CEL-generated list by exact position — use item-level `(?...)` matching instead.** Any list field produced by a CEL `.merge().transformList()` chain (ACK `spec.tags`, S3 `spec.tagging`, KMS `tagKey`/`tagValue` tags, etc.) has map-derived iteration order that is not guaranteed stable across runs — a positional assert is a latent flake, not just a guessing problem. Always pair a `(length(field)): N` check with one `- (key == 'x'): true` (or `tagKey`/`name`, whichever field is unique per element) per expected item. See `docs/frequent-rgd-errors.md` §6 "Flaky List/Array Asserts — CEL Map-to-List Transforms Have Unstable Order". Exception: lists that echo raw user input verbatim (e.g. `allowedKeySpecs`, `policies` ARNs) are apply-order-stable and may use plain positional asserts.
