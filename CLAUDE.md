...
- **`{namespace}-{name}` naming template** — default in every `<Kind>Config`. Chainsaw tests run in namespace `<testset>` (e.g. `iamrole`), so expected cloud resource names are `<namespace>-<resource>` (e.g. `iamrole-ec2-role`), not always `default-*`.
...
- **An RGD edit is not verified until the RGD reaches `Active` in a cluster — and kro only re-validates on re-CREATE.** `kubectl apply` over an unchanged-generation RGD (or an annotation-only "force resync") does NOT re-run graph validation, and a long-failing RGD's controller backoff is capped at ~1000s, so the stale `GraphAccepted` message can persist for ~16 minutes and read as "my fix didn't work". Always `kubectl delete rgd <name>` then `kubectl apply -f rgds/<name>.yaml`, then read `.status.conditions[?(@.type=="GraphAccepted")].message`. Expect errors to surface **one layer at a time** — budget several rounds. Reasoning about CEL types on paper is not verification. See `docs/frequent-rgd-errors.md` §8.
- **Verify every field name AND type against the live ACK CRD, never against intuition.** kro type-checks templates against the ACK OpenAPI schema, so any drift makes the RGD `Inactive` before an instance exists. Real KRO-443 drift: `responseCode` is a **string** not an integer; the field is `httpSPort` (capital S) not `httpsPort`; `cachedMethods` is nested under `allowedMethods`, not flat. Dump the target shape first with `kubectl get crd <plural>.<service>.services.k8s.aws -o json | jq '...properties | map_values(.type)'`. ACK fields typed `string, format: byte` (e.g. `Function.spec.functionCode`) map to CEL `bytes` and **must** be wrapped: `${bytes(schema.spec.functionCode)}` — string pass-through is rejected outright. See `docs/frequent-rgd-errors.md` §7.
- **`.orValue()` cannot take a map literal for a nested named-type field, and `transformList` must use the 3-arg form.** Inside a macro, `o.?s3OriginConfig` is `optional(<named struct>)`; `.orValue({"k":""})` fails with `found no matching overload for 'orValue'`. Chain `.?` down to a **scalar leaf** and `orValue()` only that scalar, building each ternary branch as an explicit map literal. Separately, kro's dependency extractor only understands `transformList(indexVar, valueVar, expr)` — the 2-arg form fails with `references unknown identifiers: [<var>]`.
- **`kubectl wait rgd --all --timeout=120s` shares ONE budget across all RGDs** — the CI setup failure list is "first broken RGD + everything alphabetically after it", not the set of broken RGDs. Ignore the list; run `kubectl get rgd --no-headers | awk '$5!="True"{print $1, $4, $5}'` to find the real ones. `tests/setup.sh` now prints this diagnosis automatically on failure.
...
- **Purge step must delete every resource tier the test owns** — if a test uses both `KropathConfig` and `IAMConfig`, both must appear in `purge-stale-leftovers`. Omitting one tier leaves stale state that makes the test appear to pass in isolation while masking a wrong assert.
