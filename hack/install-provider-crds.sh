...
ACK_SERVICES="${ACK_SERVICES:-s3 iam kms ec2 dynamodb dsql rds sns sqs ssm secretsmanager eks ecr cloudwatch cloudwatchlogs elbv2 eventbridge autoscaling applicationautoscaling cloudfront ecs apigateway apigatewayv2 lambda kafka elasticache efs memorydb sfn acm acmpca glue athena route53 route53resolver cognitoidentityprovider kinesis cloudtrail documentdb keyspaces pipes opensearchservice opensearchserverless ses bedrock bedrockagent bedrockagentcorecontrol sagemaker networkfirewall mwaa mq ram emrcontainers emrserverless}"
...
if [ "${SKIP_ACK}" = "false" ]; then
  ...
  for service in ${ACK_SERVICES}; do
    ...
    if ! helm pull "oci://${ACK_REGISTRY}/${service}-chart" \
      --version "${version}" --untar --untardir tmp/ 2>/dev/null; then
      echo "    WARNING: Could not pull ACK chart for ${service}:${version} — chart not found in ECR, skipping."
      continue
    fi
